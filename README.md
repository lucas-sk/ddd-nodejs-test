...
- Quais as ações (casos de uso) que essa aplicação deve ter?
    - rastrear um produto individualmente
    - definir quantidades mínimas produtos no estoque
    - visualização de históricos de vendas e estoque
    - criação/edição de um produto (identificação unica e informações extras)
    - criar/gerenciar pedidos de compra
